...
!! channel storage and from volume flowing out
       ...
       !rttlc = 0. !no longer necessary now that rttlc is an array, !tcw

	  if (rtwtr > 0.) then

	!!  Total time in hours to clear the water

          rttlc(jrch) = det * ch_k(2,jrch) * ch_l2(jrch) * p !tcw
          rttlc2 = rttlc(jrch) * rchstor(jrch) / (rtwtr + rchstor(jrch)) !tcw

	    if (rchstor(jrch) <= rttlc2) then
	      rttlc2 = min(rttlc2, rchstor(jrch))
	      rchstor(jrch) = rchstor(jrch) - rttlc2
	      rttlc1 = rttlc(jrch) - rttlc2 !tcw
